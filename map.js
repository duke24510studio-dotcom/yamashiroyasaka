...
import { getConfiguredCsvSource, loadRecords, parseRecordCsv, readCsvFile } from './data.js';
...
async function loadConfiguredSource() {
  try {
    ...
    setSourceStatus(activeSource.label, loadedMessage(records.length));
    render();
  } catch (error) {
...
async function loadUserCsv(event) {
  ...
  records = parseRecordCsv(await readCsvFile(file));
  selectedId = null;
  activeSource = { source: file.name, label: `手元のCSVファイル: ${file.name}` };
  resetFilterValues();
  populateFilters(records, elements);
  setSourceStatus(activeSource.label, loadedMessage(records.length));
  render();
...
function loadedMessage(count) {
  if (count === 0) {
    return '0件です。CSVの列名、緯度、経度を確認してください。';
  }

  return `${count}件の${appConfig.sourceName}を読み込みました。`;
}
